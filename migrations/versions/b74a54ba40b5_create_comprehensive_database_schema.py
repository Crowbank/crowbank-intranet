...
def upgrade() -> None:
    ...
    # First, create the enum types we need with checkfirst=True
    boardingtype = sa.Enum('DOG_STANDARD', 'DOG_DELUXE', 'DOG_DOUBLE', 'CATTERY', name='new_boardingtype')
    
    bookingstatus = sa.Enum('PENDING', 'CONFIRMED', 'CHECKED_IN', 'CHECKED_OUT', 'CANCELLED', 'NO_SHOW', name='new_bookingstatus')
    
    boardingtype2 = sa.Enum('STANDARD', 'DELUXE', 'CATTERY', 'DAYCARE', name='new_boardingtype2')
    
    documenttype = sa.Enum('VACCINATION', 'INSTRUCTIONS', 'MEDICATION_SHEET', 'VET_VISIT_SHEET', 'OTHER', name='new_documenttype')
    
    mediatype = sa.Enum('PHOTO', 'VIDEO', name='new_mediatype')
    
    formtype = sa.Enum('NEW_DOG_QUESTIONNAIRE', 'DAYCARE_APPLICATION', 'PUPPY_CLASS_REGISTRATION', 'CHECKIN_FORM', 'BOOKING_INTENT', 'EMPLOYEE_STARTER', name='new_formtype')
    
    entitytype = sa.Enum('PET', 'BOOKING', 'CUSTOMER', 'EMPLOYEE', name='new_entitytype')
    
    fieldtype = sa.Enum('TEXT', 'TEXTAREA', 'NUMBER', 'DATE', 'SELECT', 'MULTISELECT', 'CHECKBOX', 'RADIO', 'FILE', 'EMAIL', 'PHONE', 'ADDRESS', name='new_fieldtype')
    
    # Now create base contact, vet, customer tables needed for FK dependencies
    ...
    op.create_table('customers',
        sa.Column('id', sa.Integer(), nullable=False),
        sa.Column('legacy_cust_no', sa.Integer(), nullable=True),
        sa.Column('street', sa.String(length=255), nullable=True),
        sa.Column('town', sa.String(length=100), nullable=True),
        sa.Column('postcode', sa.String(length=20), nullable=True),
        sa.Column('default_vet_id', sa.Integer(), nullable=True),
        sa.ForeignKeyConstraint(['default_vet_id'], ['vets.id']),
        sa.PrimaryKeyConstraint('id'),
        sa.UniqueConstraint('legacy_cust_no')
    )
    ...
    # Now create species and other tables first as normal
    ...
    sa.Column('type', sa.Enum('DOG_STANDARD', 'DOG_DELUXE', 'DOG_DOUBLE', 'CATTERY', name='new_boardingtype'), nullable=False),
    ...
    sa.Column('type', sa.Enum('NEW_DOG_QUESTIONNAIRE', 'DAYCARE_APPLICATION', 'PUPPY_CLASS_REGISTRATION', 'CHECKIN_FORM', 'BOOKING_INTENT', 'EMPLOYEE_STARTER', name='new_formtype'), nullable=False),
    sa.Column('entity_type', sa.Enum('PET', 'BOOKING', 'CUSTOMER', 'EMPLOYEE', name='new_entitytype'), nullable=False),
    ...
    sa.Column('description', sa.Text(), nullable=True),
    sa.Column('type', sa.Enum('PHOTO', 'VIDEO', name='new_mediatype'), nullable=False),
    ...
    sa.Column('type', sa.Enum('VACCINATION', 'INSTRUCTIONS', 'MEDICATION_SHEET', 'VET_VISIT_SHEET', 'OTHER', name='new_documenttype'), nullable=False),
    sa.Column('original_filename', sa.String(length=255), nullable=True),
    ...
    sa.Column('field_type', sa.Enum('TEXT', 'TEXTAREA', 'NUMBER', 'DATE', 'SELECT', 'MULTISELECT', 'CHECKBOX', 'RADIO', 'FILE', 'EMAIL', 'PHONE', 'ADDRESS', name='new_fieldtype'), nullable=False),
    ...
    sa.Column('status', sa.Enum('PENDING', 'CONFIRMED', 'CHECKED_IN', 'CHECKED_OUT', 'CANCELLED', 'NO_SHOW', name='new_bookingstatus'), nullable=False),
    sa.Column('boarding_type', sa.Enum('STANDARD', 'DELUXE', 'CATTERY', 'DAYCARE', name='new_boardingtype2'), nullable=False),
...
def downgrade() -> None:
    ...
    # Clean up by dropping enum types at the end
    sa.Enum(name='new_fieldtype').drop(op.get_bind(), checkfirst=True)
    sa.Enum(name='new_entitytype').drop(op.get_bind(), checkfirst=True)
    sa.Enum(name='new_formtype').drop(op.get_bind(), checkfirst=True)
    sa.Enum(name='new_mediatype').drop(op.get_bind(), checkfirst=True)
    sa.Enum(name='new_documenttype').drop(op.get_bind(), checkfirst=True)
    sa.Enum(name='new_boardingtype2').drop(op.get_bind(), checkfirst=True)
    sa.Enum(name='new_boardingtype').drop(op.get_bind(), checkfirst=True)
    sa.Enum(name='new_bookingstatus').drop(op.get_bind(), checkfirst=True)
    # ### end Alembic commands ###
