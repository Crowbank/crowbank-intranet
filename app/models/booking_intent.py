...
booking_intent_pets = Table(
    ...
    Base.metadata,
    Column(
        "booking_intent_id", Integer, ForeignKey("booking_intents.id"), primary_key=True
    ),
...
class BookingIntent(Base):
    ...
    arrival_period = Column(
        Enum(DayPeriod), nullable=True
    )  # AM/PM preference for arrival
    departure_period = Column(
        Enum(DayPeriod), nullable=True
    )  # AM/PM preference for departure
    ...
    status = Column(
        Enum(BookingIntentStatus), nullable=False, default=BookingIntentStatus.OPEN
    )
    ...
    form_submission_id = Column(
        Integer, ForeignKey("form_submissions.id"), nullable=True
    )
    ...
    # Relationships
    pets = relationship(
        "Pet", secondary=booking_intent_pets, back_populates="booking_intents"
    )
    ...
    duplicates = relationship(
        "BookingIntent", foreign_keys=[duplicate_of_id], backref="duplicate_of"
    )
