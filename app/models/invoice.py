...
class Invoice(Base):
    ...
    charges = relationship(
        "Charge", back_populates="invoice", cascade="all, delete-orphan"
    )
    ...
    gross_amount = column_property(
        func.coalesce(func.sum(func.coalesce("Charge.amount", 0)), 0).label(
            "gross_amount"
        )
    )
...
class Charge(Base):
    ...
    )  # For multiple days or units
    is_quantity_overridden = Column(
        Boolean, default=False, nullable=False
    )  # Whether quantity was manually set
    is_rate_overridden = Column(
        Boolean, default=False, nullable=False
    )  # Whether rate was manually set
    is_peak = Column(
        Boolean, default=False, nullable=False
    )  # Whether this is a peak date charge (for premium pricing)
    minimum_days = Column(
        Integer, nullable=True
    )  # Booking-specific minimum days override
    notes = Column(Text, nullable=True)
