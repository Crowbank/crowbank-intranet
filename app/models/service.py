...
class Service(Base):
    ...
    )  # Default to double rate for peak dates

    # Minimum days for boarding services
    min_days_standard = Column(
        Integer, nullable=True
    )  # Minimum days in standard season
    ...
    # Relationships
    rates = relationship(
        "ServiceRate", back_populates="service", cascade="all, delete-orphan"
    )
    ...
    def get_min_days(self, season=Season.STANDARD):
        ...
        """
        if (
            self.service_type != ServiceType.BOARDING
            and self.applicability != ServiceApplicability.PER_PET_DAY
        ):
            return None
...
class ServiceRate(Base):
    ...
    season = Column(
        Enum(Season), default=Season.STANDARD, nullable=False
    ...
    def __repr__(self):
        season_info = (
            f", season={self.season}" if self.season != Season.STANDARD else ""
        )
