...
class FormTemplate(Base):
    ...
    )  # What this form relates to (pet, booking, etc.)
    gravity_form_id = Column(
        Integer, nullable=True
    )  # ID of the corresponding Gravity Form
...
class FormField(Base):
    ...
    )  # Machine-readable key (e.g., "pet_name")
    label = Column(
        String(255), nullable=False
    )  # Human-readable label (e.g., "Pet's Name")
    ...
    options = Column(
        JSON, nullable=True
    )  # For select/multiselect/radio/checkbox options
...
class FormSubmission(Base):
    ...
    gravity_entry_id = Column(
        Integer, nullable=True
    )  # ID of the entry in Gravity Forms
    ...
    responses = relationship(
        "FormResponse", back_populates="submission", cascade="all, delete-orphan"
    )
    pet = relationship("Pet", foreign_keys=[pet_id], back_populates="form_submissions")
    booking = relationship(
        "Booking", foreign_keys=[booking_id], back_populates="form_submissions"
    )
    customer = relationship(
        "Customer", foreign_keys=[customer_id], back_populates="form_submissions"
    )
    submitted_by = relationship("Customer", foreign_keys=[submitted_by_id])
    files = relationship(
        "FormFile", back_populates="submission", cascade="all, delete-orphan"
    )
