...
class Pet(Base):
    ...
    friends_allowed = Column(
        Boolean, default=True, nullable=False
    )  # Can interact with other pets
    daycare_approved = Column(
        Boolean, default=False, nullable=False
    )  # Approved for daycare service
    ...
    vaccination_document_id = Column(
        Integer, ForeignKey("pet_documents.id"), nullable=True
    )
    ...
    booking_intents = relationship(
        "BookingIntent", secondary="booking_intent_pets", back_populates="pets"
    )
    vaccination_events = relationship("VaccinationEvent", back_populates="pet")
    documents = relationship(
        "PetDocument", back_populates="pet", foreign_keys="PetDocument.pet_id"
    )
    ...
    vaccination_document = relationship(
        "PetDocument", foreign_keys=[vaccination_document_id]
    )
    form_submissions = relationship(
        "FormSubmission", foreign_keys="FormSubmission.pet_id", back_populates="pet"
    )

    # Many-to-many relationship with media files
    media_files = relationship(
        "MediaFile", secondary=pet_media_association, back_populates="pets"
    )

    # Many-to-many relationship with medical conditions
    medical_conditions = relationship(
        "MedicalCondition", secondary=pet_condition_association, back_populates="pets"
    )
    ...
    def current_weight(self):
        ...
        latest_weight = sorted(
            self.weights, key=lambda w: w.date_recorded, reverse=True
        )[0]
    ...
    def has_completed_form(self, form_type):
        ...
        """
        return any(
            s.is_complete and s.template.type == form_type
            for s in self.form_submissions
        )
    ...
    def set_vaccination_card(self, document):
        ...
        if document.type != DocumentType.VACCINATION:
            raise ValueError(
                "Document must be of type VACCINATION to be set as vaccination card"
            )
