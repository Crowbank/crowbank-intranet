...
class MediaService:
    ...
    def upload_media(
        ...
        if bucket_name is None:
            ...
            if not bucket_name and "CONFIG" in current_app.config:
                ...
                if not bucket_name:
                    bucket_name = nested_config.get("storage", {}).get(
            ...
            if not bucket_name:
                ...
                raise ValueError(
                    "Media storage bucket not configured. Check application configuration."
                )
        ...
        if not media_type:
            ...
            if content_type and content_type.startswith("image/"):
                ...
                if ext in [".jpg", ".jpeg", ".png", ".gif", ".webp", ".bmp"]:
                    ...
                    raise ValueError(
                        f"Could not determine media type for file: {file.filename}"
                    )
    ...
    def associate_with_pets(
        db_session: Session, media_id: int, pet_ids: list[int]
    ) -> bool:
        """
        ...
        try:
            ...
            current_app.logger.error(f"Error associating media with pets: {e!s}")
            db_session.rollback()
            return False

    @staticmethod
    def remove_pet_association(db_session: Session, media_id: int, pet_id: int) -> bool:
        """
        ...
        try:
            ...
            current_app.logger.error(f"Error removing media-pet association: {e!s}")
            db_session.rollback()
            return False

    @staticmethod
    def get_media_for_pet(
        db_session: Session, pet_id: int, media_type: str | None = None
    ) -> list[MediaFile]:
        """
        ...
        """
        query = (
            db_session.query(MediaFile).join(MediaFile.pets).filter(Pet.id == pet_id)
        )
    ...
    @staticmethod
    def get_media_file(db_session: Session, media_id: int) -> MediaFile | None:
        """
    ...
    @staticmethod
    def get_media_url(
        media_file: MediaFile, thumbnail: bool = False, expires_in: int = 3600
    ) -> str:
        """
    ...
    def delete_media(db_session: Session, media_id: int) -> bool:
        ...
        success = cloud_storage.delete_file(
            bucket_name=media_file.bucket_name, object_key=media_file.object_key
        )

        # Also delete thumbnail if it exists
        if success and media_file.thumbnail_key:
            cloud_storage.delete_file(
                bucket_name=media_file.bucket_name, object_key=media_file.thumbnail_key
            )
