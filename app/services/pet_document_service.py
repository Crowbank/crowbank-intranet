...
from app.models import DocumentType, Pet, PetDocument
...
class PetDocumentService:
    ...
    def upload_document(
        ...
        if bucket_name is None:
            ...
            env = current_app.config.get(
                "ENV", "dev"
            )  # Using shortened env name (dev/prod/test)
            ...
            bucket_name = current_app.config.get(
                f"STORAGE_BUCKETS_PET_DOCUMENTS_{env.upper()}"
            )
            ...
            if not bucket_name:
                ...
                raise ValueError(
                    "Cloud storage bucket not configured. Check application configuration."
                )
    ...
    def get_documents_by_type(
        db_session: Session, pet_id: int, document_type: DocumentType
    ) -> list[PetDocument]:
        """
        ...
        """
        return (
            db_session.query(PetDocument)
            .filter(PetDocument.pet_id == pet_id, PetDocument.type == document_type)
            .all()
        )

    @staticmethod
    def get_vaccination_document(
        db_session: Session, pet_id: int
    ) -> PetDocument | None:
        """
    ...
    @staticmethod
    def set_vaccination_document(
        db_session: Session, pet_id: int, document_id: int
    ) -> bool:
        """
